...
// Documentation of the algorithm taken verbatim from the MIT source file:
/*
   ...
   REVISION HISTORY:
     ...
     18 December 2012: Slight tweaks (remove recomputation of x*x in Dawson)
          ...
          21 Mar 2017: Ported to Java by crf
*/
/**
 * This is a port of [http://ab-initio.mit.edu/Faddeeva.cc](http://ab-initio.mit.edu/Faddeeva.cc)
 * converted from C++ to Java.  The original code is covered by the MIT license.
 */
object Erf {
    private const val USE_CONTINUED_FRACTION = true

    /**
     * The smallest double value E such that (1 + E) does not equal 1.
     */
    private const val DBL_EPSILON = 2.2204460492503131e-16
    private const val PI = 3.1415926535897932384626433832795028841972
    private const val SQRT_PI_OVER_2 = 0.8862269254527580136490837416705725913990
    private const val ONE_OVER_SQRT_PI = 0.56418958354775628694807945156

    // Constants used by original author during "fit"
    private const val FIT_C0 = 3.9
    private const val FIT_C1 = 11.398
    private const val FIT_C2 = 0.08254
    private const val FIT_C3 = 0.1421
    private const val FIT_C4 = 0.2023

    // precomputed table of expa2n2[n-1] = exp(-a2*n*n)
    // for double-precision a2 = 0.26865... in w(Complex, double).
    private val EXPA2N2 = doubleArrayOf(
    ...
    fun erf(x: Double): Double {
        when {
            x == 0.0 -> return 0.0
            x.isNaN() -> return Double.NaN
        }

        val mx2 = x * -x
        if (mx2 < -750) {
            // underflow
            return 1.0.withSign(x)
        }
        if (x >= 0) {
            return if (x < 0.08) erfTaylor(x, mx2) else 1.0 - exp(mx2) * erfcx(x)
        }
        return if (x > -0.08) erfTaylor(x, mx2) else exp(mx2) * erfcx(-x) - 1.0
    }

    private fun erfTaylor(x: Double, mx2: Double): Double {
    ...
    fun erf(z: Complex, relerr: Double = DBL_EPSILON): Complex {
        val x = z.x
        val y = z.y
        if (y == 0.0) return Complex(erf(x), y)  // preserve sign of imaginary 0
        if (x == 0.0) {
            /* handle y -> Inf limit manually, since
               exp(y^2) -> Inf but Im[w(y)] -> 0, so
               IEEE will give us a NaN when it should be Inf */
            val y2 = y * y
            val w = if (y2 > 720) Double.POSITIVE_INFINITY.withSign(y) else exp(y2) * wIm(y)
            return Complex(x, w)  // preserve sign of real 0
        }
        val mz2re = (y - x) * (x + y) // Re(-z^2), being careful of overflow
        val mz2im = x * -2 * y // Im(-z^2)
        if (mz2re < -750) {  // underflow
            return real(1.0.withSign(x))
        }
        ...
        if (x >= 0) {
            if (x < 0.08) {
                if (abs(y) < 0.01) {
                    return erfTaylor(z, mz2re, mz2im)
                }
                ...
                val w = Complex(cos(mz2im), sin(mz2im)) * w(z.timesJ, relerr) * exp(mz2re)
                return -w + 1.0
            }
        } else if (x > -0.08) { // duplicate from above to avoid abs(x) call
            if (abs(y) < 0.01) {
                return erfTaylor(z, mz2re, mz2im)
            }
            if (abs(mz2im) < 0.005 && x > -0.005) {
        ...
        /* don't use complex exp function, since that will produce spurious NaN
           values when multiplying w in an overflow situation. */return Complex(
            cos(mz2im),
            sin(mz2im)
        )
            .times(exp(mz2re))
    ...
    private fun erfTaylor(z: Complex, mz2re: Double, mz2im: Double): Complex {
        val mz2 = Complex(mz2re, mz2im) // -z^2
        return z.times(
            sum(
                            ...
                            sum(
                                ...
                                mz2.times(
                                    sum(
                                        ...
                                        mz2.times(0.0052239776254421878422)
                                    )
                                )
                            )
                        )
    ...
    */
    private fun erfiTaylor(x: Double, y: Double): Complex {
        val x2 = x * x
        val y2 = y * y
        val expy2 = exp(y2)
        return Complex(
    ...
    // erfi(z) = -i erf(iz)
    fun erfi(z: Complex, relerr: Double = DBL_EPSILON): Complex {
        return erf(z.timesJ, relerr).timesNegativeJ
    }

    // erfi(x) = -i erf(ix)
    fun erfi(x: Double): Double {
        val x2 = x * x
        return if (x2 > 720) Double.POSITIVE_INFINITY.withSign(x) else exp(x2) * wIm(x)
    }
    ...
    fun erfc(x: Double): Double {
        ...
        return if (x >= 0) {
            exp(mx2) * erfcx(x)
        } else 2 - exp(mx2) * erfcx(mx)
    }

    // erfc(z) = 1 - erf(z)
    fun erfc(z: Complex, relerr: Double = DBL_EPSILON): Complex {
        val x = z.x
        val y = z.y
        return when {
            x == 0.0 -> {
                ...
                val y2 = y * y
                when {
                    y2 > 720 -> Complex(  // underflow
                        x = 1.0,
                        y = if (y > 0) Double.NEGATIVE_INFINITY else Double.POSITIVE_INFINITY
                    )
                    else -> Complex(1.0, -exp(y2) * wIm(y))
                }
            }
            ...
            y == 0.0 -> {
                val x2 = x * x
                when {
                    x2 > 750 -> Complex(if (x >= 0) 0.0 else 2.0, -y)  // underflow; preserve sign of 0
                    else -> Complex(
                        x = if (x >= 0) exp(-x2) * erfcx(x) else 2 - exp(-x2) * erfcx(-x),
                        y = -y  // preserve correct sign of zero
                    )
                }
            }
                ...
                val mz2re = (y - x) * (x + y) // Re(-z^2), being careful of overflow
                val mz2im = -2 * x * y // Im(-z^2)

                when {
                    mz2re < -750 -> if (x >= 0) Complex.ZERO else real(2.0)  // underflow
                    x >= 0 -> Complex(mz2re, mz2im).exp * w(z.timesJ, relerr)
                    else -> Complex(mz2re, mz2im).exp * -w(z.timesNegativeJ, relerr) + 2.0
                }
            }
        }
    }

    fun dawson(x: Double): Double {
        return SQRT_PI_OVER_2 * wIm(x)
    }

    // compute Dawson(z) = sqrt(pi)/2  *  exp(-z^2) * erfi(z)
    fun dawson(z: Complex, relerr: Double): Complex {
        val x = z.x
        val y = z.y

        // handle axes separately for speed & proper handling of x or y = Inf or NaN
        if (y == 0.0) return Complex(SQRT_PI_OVER_2 * wIm(x), -y)  // preserve sign of 0
        ...
        if (x == 0.0) {
            ...
            if (y2 < 2.5e-5) { // Taylor expansion
                ...
                return Complex(x, im) // preserve sign of 0
            }
            val im = SQRT_PI_OVER_2 * if (y >= 0) exp(y2) - erfcx(y) else erfcx(-y) - exp(y2)
            return Complex(x, im)
        }

        val mz2 = Complex(
            x = (y - x) * (x + y),  // Re(-z^2), being careful of overflow
            y = -2 * x * y  // Im(-z^2)
        )
        /* Handle positive and negative x via different formulas,
           using the mirror symmetries of w, to avoid overflow/underflow
        ...
        if (y >= 0) {
            if (y < 0.005) {
                if (x.absoluteValue < 5e-3) {
                    return dawsonTaylor(z, mz2)
                }
                if (mz2.y.absoluteValue < 0.005) {
                    return dawsonTaylorRealAxis(x, y)
                }
            }
            val res = mz2.exp.minus(w(z, relerr))
            return res.timesJ.times(SQRT_PI_OVER_2)
        }

        // y < 0
        if (y > -0.005) { // duplicate from above to avoid abs(x) call
            if (x.absoluteValue < 0.005) {
                return dawsonTaylor(z, mz2)
            }
            if (mz2.y.absoluteValue < 0.005) {
                return dawsonTaylorRealAxis(x, y)
            ...
            // The .cc source checked x == 0 here, but that was ruled out above
            return Complex.NaN
        }
        val res = w(z.negative, relerr).minus(mz2.exp)
        return res.timesJ.times(SQRT_PI_OVER_2)
    }

    // Use Taylor series for small |z|, to avoid cancellation inaccuracy
    //     dawson(z) = z - 2/3 z^3 + 4/15 z^5 + ...
    private fun dawsonTaylor(z: Complex, mz2: Complex): Complex {
        return z.times(
            sum(
                1.0,
                mz2.times(
                    sum(
                        ...
                        mz2.times(0.2666666666666666666666666666666666666667)
                    )
                )
            )
        )
 ...
 */
    private fun dawsonTaylorRealAxis(x: Double, y: Double): Complex {
        val x2 = x * x
        return if (x2 > 1600) { // |x| > 40
            val y2 = y * y
            if (x2 > 25e14) { // |x| > 5e7
                val xy2 = x * y * (x * y)
                return Complex(
                    (0.5 + y2 * (0.5 + 0.25 * y2
                            - 0.16666666666666666667 * xy2)) / x, y * (-1 + y2 * (-0.66666666666666666667
            ...
            val D = SQRT_PI_OVER_2 * wIm(x)
            val y2 = y * y
            Complex(
                D + y2 * (D + x - 2 * D * x2) + y2 * y2 * (D * (0.5 - x2 * (2 - 0.66666666666666666667 * x2))
                        + x * (0.83333333333333333333
                        - 0.33333333333333333333 * x2)),
                y * (1 - 2 * D * x + y2 * 0.66666666666666666667 * (1 - x2 - D * x * (3 - 2 * x2)) + y2 * y2 * (0.26666666666666666667 - x2 * (0.6 - 0.13333333333333333333 * x2) - D * x * (1 - x2 * (1.3333333333333333333
                        - 0.26666666666666666667 * x2))))
            )
        }
    }

    private fun sinc(x: Double, sinx: Double): Double {
        return if (abs(x) < 1e-4) 1 - 0.1666666666666666666667 * x * x else sinx / x
    }

    // sinh(x) via Taylor series, accurate to machine precision for |x| < 1e-2
    private fun sinhTaylor(x: Double): Double {
        val x2 = x * x
        return x * (1 + x2 * (0.1666666666666666666667 + 0.00833333333333333333333 * x2))
    }
    ...
    fun w(z: Complex, relerr: Double = DBL_EPSILON): Complex {
        var rerr = relerr
        var x = z.x
        val y = z.y
        if (x == 0.0) {
        ...
        if (rerr <= DBL_EPSILON) {
            ...
            if (rerr > 0.1) rerr = 0.1 // not sensible to compute < 1 digit
            a = PI / sqrt(-ln(rerr * 0.5))
            c = 2 / PI * a
            a2 = a * a
        }
        x = abs(x)
        ...
        // continued fraction is faster for x > 6
        /* As pointed out by M. Zaghloul, the continued
           ...
           algorithm 816 in this region: */if (USE_CONTINUED_FRACTION) {
            if (ya > 7 || x > 6 && (ya > 0.1 || x > 8 && ya > 1e-10 || x > 28)) {
               ...
               I also separate the regions where nu == 2 and nu == 1. */
                val xs = if (y < 0) -z.x else z.x // compute for -z if y < 0
                if (x + ya > 4000) { // nu <= 2
                    ret = if (x + ya > 1e7) { // nu == 1, w(z) = i/sqrt(pi) / z
                        ...
                        if (x > ya) {
                            ...
                            Complex(denom, denom * xya)
                        }
                    } else { // nu == 2, w(z) = i/sqrt(pi) * z / (z*z - 0.5)
                        val dr = xs * xs - ya * ya - 0.5
                        val di = 2 * xs * ya
                        val denom = ONE_OVER_SQRT_PI / (dr * dr + di * di)
                        Complex(denom * (xs * di - ya * dr), denom * (xs * dr + ya * di))
                    }
                } else { // compute nu(z) estimate and do general continued fraction
                    var nu = floor(FIT_C0 + FIT_C1 / (FIT_C2 * x + FIT_C3 * ya + FIT_C4))
                    var wr = xs
                    var wi = ya
                    nu = 0.5 * (nu - 1)
                    while (nu > 0.4) {
                    ...
                    run {
                        // w(z) = i/sqrt(pi) / w:
                        val denom = ONE_OVER_SQRT_PI / (wr * wr + wi * wi)
                ...
                return if (y < 0) {
                    ...
                    Complex((ya - xs) * (xs + ya), 2 * xs * y).exp * 2.0 - ret
                } else ret
            }
     ...
     below.  Therefore, we use x < 10 here. */if (x < 10) {
            ...
            /* Somewhat ugly copy-and-paste duplication here, but I see significant
               ...
               exponential, and the x < 5e-4 special case is needed for accuracy. */if (rerr == DBL_EPSILON) { // use precomputed exp(-a2*(n*n)) table
                if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
                    ...
                    val ax2 = 1.036642960860171859744 * x // 2*a*x
                    val exp2ax = 1 + ax2 * (1 + ax2 * (0.5 + 0.166666666666666666667 * ax2))
                    val expm2ax = 1 - ax2 * (1 - ax2 * (0.5 - 0.166666666666666666667 * ax2))
                    var n = 1
                    while (true) {
                        val coef = EXPA2N2[n - 1] * expx2 / (a2 * (n * n) + y * y)
                        prod2ax *= exp2ax
                        prodm2ax *= expm2ax
                        sum1 += coef
                        sum2 += coef * prodm2ax
                    ...
                    expx2 = exp(-x * x)
                    val exp2ax = exp(2 * a * x)
                    val expm2ax = 1 / exp2ax
                    var n = 1
                    while (true) {
                        val coef = EXPA2N2[n - 1] * expx2 / (a2 * (n * n) + y * y)
                        prod2ax *= exp2ax
                ...
                if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
                    ...
                    var n = 1
                    while (true) {
                        val coef = exp(-a2 * (n * n)) * expx2 / (a2 * (n * n) + y * y)
                        prod2ax *= exp2ax
                        prodm2ax *= expm2ax
                        sum1 += coef
                        sum2 += coef * prodm2ax
            ...
            val expx2erfcxy = if (y > -6) expx2 * erfcx(y) else 2 * exp(y * y - x * x)
            ret = if (y > 5) { // imaginary terms cancel
                val sinxy = sin(x * y)
                real(
                    (expx2erfcxy - c * y * sum1) * cos(2 * x * y)
                            + c * x * expx2 * sinxy * sinc(x * y, sinxy)
                )
            } else {
                val xs = z.x
                val sinxy = sin(xs * y)
                val sin2xy = sin(2 * xs * y)
                val cos2xy = cos(2 * xs * y)
                val coef1 = expx2erfcxy - c * y * sum1
                val coef2 = c * xs * expx2
                Complex(
            ...
            ret = if (USE_CONTINUED_FRACTION) {
                ...
                if (y < 0) {
                   ...
                   in the case where y is very small. */
                    Complex.polar(2 * exp(y * y - x * x) - exp(-x * x), -2 * z.x * y)
                } else {
                    real(exp(-x * x)) // not negligible in real part if y very small
                }
            }

            // (round instead of ceil as in original paper; note that x/a > 1 here)
            val n0 = floor(x / a + 0.5) // sum in both directions, starting at n0
            val dx = a * n0 - x
            sum3 = exp(-dx * dx) / (a2 * (n0 * n0) + y * y)
            sum5 = a * n0 * sum3
            val exp1 = exp(4 * a * dx)
            var exp1dn = 1.0
            var useWhileLoop = true
            var dn: Int
            dn = 1
            while (n0 - dn > 0) {
                ...
                val np = n0 + dn
                val nm = n0 - dn
                var tp = exp(-sqr(a * dn + dx))
                var tm = tp * exp1.let { exp1dn *= it; exp1dn } // trick to get tm from tp
                tp /= a2 * (np * np) + y * y
                tm /= a2 * (nm * nm) + y * y
                sum3 += tp + tm
                sum5 += a * (np * tp + nm * tm)
                if (a * (np * tp + nm * tm) < rerr * sum5) {
                    useWhileLoop = false
                    break
                }
                ++dn
            }
            while (useWhileLoop) { // loop over n0+dn terms only (since n0-dn <= 0)
                val np = n0 + dn++
                val tp = exp(-sqr(a * dn + dx)) / (a2 * (np * np) + y * y)
                sum3 += tp
                sum5 += a * np * tp
                if (a * np * tp < rerr * sum5) {
                    break
                }
            }
        }
        return ret.plus(
            Complex(
                x = 0.5 * c * y * (sum2 + sum3),
                y = 0.5 * c * (sum5 - sum4).withSign(z.x)
            )
        )
    }

    // For simplifying places where + or * were used on (double +/* complex) pairs in a taylor series
    private fun sum(x: Double, z: Complex): Complex {
        return z.plus(x)
    }

    private fun prod(x: Double, z: Complex): Complex {
        return z.times(x)
    }
    ...
    fun erfcxY100(y100: Double): Double {
        when (y100.toInt()) {
            0 -> {
                val t = 2 * y100 - 1
    ...
    fun erfcx(z: Complex, relerr: Double = DBL_EPSILON): Complex {
        return w(z.timesJ, relerr)
   }

    fun erfcx(value: Double): Double {
        return if (value >= 0) {
            if (value > 50) { // continued-fraction expansion is faster
                if (value > 5e7) { // 1-term expansion, important to avoid overflow
                    return ONE_OVER_SQRT_PI / value
                }
                /* 5-term expansion (rely on compiler for CSE), simplified from:
                     1 / sqrt(pi) / (x+0.5/(x+1/(x+1.5/(x+2/x))))  */
                val value2 = value * value
                return ONE_OVER_SQRT_PI *
                        (value2 * (value2 + 4.5) + 2) /
                        (value * (value2 * (value2 + 5) + 3.75))
            }
            erfcxY100(400 / (4 + value))
        } else if (value < -26.7) {
            Double.POSITIVE_INFINITY
        } else if (value < -6.1) {
            2 * exp(value * value)
        } else {
            2 * exp(value * value) - erfcxY100(400 / (4 - value))
        }
    }

    /* Compute a scaled Dawson integral
                wIm(x) = 2*dawson(x)/sqrt(pi)
    ...
    /* Given y100=100*y, where y = 1/(1+x) for x >= 0, compute w_im(x).
       ...
       compared to fitting the whole [0,1] interval with a single polynomial. */
    private fun wImY100(y100: Double, x: Double): Double {
        when (y100.toInt()) {
            0 -> {
                val t = 2 * y100 - 1
        ...
        /* Since 0 <= y100 < 101, this is only reached if x is NaN,
           in which case we should return NaN. */return Double.NaN
    }

    private fun wIm(value: Double): Double {
        return if (value >= 0) {
            if (value > 45) { // continued-fraction expansion is faster
                if (value > 5e7) { // 1-term expansion, important to avoid overflow
                    return ONE_OVER_SQRT_PI / value
                }
                /* 5-term expansion (rely on compiler for CSE), simplified from:
                1 / sqrt(pi) / (x-0.5/(x-1/(x-1.5/(x-2/x))))  */
                val value2 = value * value
                return ONE_OVER_SQRT_PI * (value2 * (value2 - 4.5) + 2) / (value * (value2 * (value2 - 5) + 3.75))
            }
            wImY100(100 / (1 + value), value)
        } else { // = -FADDEEVA(w_im)(-x)
            if (value < -45) { // continued-fraction expansion is faster
                return if (value < -5e7) ONE_OVER_SQRT_PI / value else ONE_OVER_SQRT_PI * (value * value * (value * value - 4.5) + 2) / (value * (value * value * (value * value - 5) + 3.75))
                /* 5-term expansion (rely on compiler for CSE), simplified from:
                1 / sqrt(pi) / (x-0.5/(x-1/(x-1.5/(x-2/x))))  */
            }
            -wImY100(100 / (1 - value), -value)
