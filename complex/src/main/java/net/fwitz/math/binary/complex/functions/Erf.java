...
import static net.fwitz.math.binary.complex.Complex.polar;
...
/**
 * This is a port of <a href="http://ab-initio.mit.edu/Faddeeva.cc">http://ab-initio.mit.edu/Faddeeva.cc</a>
 * from C++ to Java.  The original code is covered by the MIT license.
 */
public class Erf {
    private static final boolean USE_CONTINUED_FRACTION = true;
    ...
    private static final double PI = 3.1415926535897932384626433832795028841972;
    private static final double SQRT_PI_OVER_2 = 0.8862269254527580136490837416705725913990;
    private static final double ONE_OVER_SQRT_PI = 0.56418958354775628694807945156;

    // Constants used by original author during "fit"
    private static final double FIT_C0 = 3.9;
    private static final double FIT_C1 = 11.398;
    private static final double FIT_C2 = 0.08254;
    private static final double FIT_C3 = 0.1421;
    private static final double FIT_C4 = 0.2023;

    // precomputed table of expa2n2[n-1] = exp(-a2*n*n)
    // for double-precision a2 = 0.26865... in w(Complex, double).
    private static final double EXPA2N2[] = {
    ...
    public static double erf(double x) {
        ...
        if (x >= 0) {
            return (x < 0.08) ? erfTaylor(x, mx2) : (1.0 - exp(mx2) * erfcx(x));
        }
        return (x > -0.08) ? erfTaylor(x, mx2) : (exp(mx2) * erfcx(-x) - 1.0);
    }

    private static double erfTaylor(double x, double mx2) {
    ...
    public static Complex erf(Complex z, double relerr) {
        ...
        if (x == 0) {
            ...
            double y2 = y * y;
            double w = (y2 > 720) ? copySign(Double.POSITIVE_INFINITY, y) : (exp(y2) * wIm(y));
            return complex(x, w);  // preserve sign of real 0
        }

        double mz2re = (y - x) * (x + y); // Re(-z^2), being careful of overflow
        double mz2im = -2 * x * y; // Im(-z^2)
        if (mz2re < -750) {  // underflow
            return real(copySign(1, x));
        }
    ...
    private static Complex erfTaylor(Complex z, double mz2re, double mz2im) {
        Complex mz2 = complex(mz2re, mz2im);  // -z^2
        return z.times(sum(1.1283791670955125739,
                mz2.times(sum(0.37612638903183752464,
                        mz2.times(sum(0.11283791670955125739,
                                mz2.times(sum(0.026866170645131251760,
    ...
    */
    private static Complex erfiTaylor(double x, double y) {
        double x2 = x * x;
        double y2 = y * y;
        double expy2 = exp(y2);
    ...
    public static double erfi(double x) {
        double x2 = x * x;
        return (x2 > 720) ? copySign(Double.POSITIVE_INFINITY, x) : (exp(x2) * wIm(x));
    }
    ...
    public static double erfc(double x) {
        ...
        if (x >= 0) {
            return exp(mx2) * erfcx(x);
        }
        return 2 - exp(mx2) * erfcx(mx);
    }


    public static Complex erfc(Complex z) {
        return erfc(z, DBL_EPSILON);
    }
    ...
    public static Complex erfc(Complex z, double relerr) {
        ...
        if (x == 0) {
            ...
            double y2 = y * y;
            if (y2 > 720) {
                return complex(1, (y > 0) ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY);
            }

            return complex(1, -exp(y2) * wIm(y));
        }

        if (y == 0) {
            double x2 = x * x;
            if (x2 > 750) {  // underflow
                return complex(x >= 0 ? 0.0 : 2.0, -y);  // preserve sign of 0
            }

            double re = (x >= 0) ? (exp(-x2) * erfcx(x)) : (2 - exp(-x2) * erfcx(-x));
            return complex(re, -y); // preserve sign of zero
        }

        double mz2re = (y - x) * (x + y); // Re(-z^2), being careful of overflow
        double mz2im = -2 * x * y; // Im(-z^2)
        if (mz2re < -750) { // underflow
            return (x >= 0) ? Complex.ZERO : real(2);
        }
    ...
    public static Complex dawson(Complex z, double relerr) {
        ...
        if (y == 0) {
            return complex(SQRT_PI_OVER_2 * wIm(x), -y);  // preserve sign of 0
        }
        ...
        if (x == 0) {
            ...
            double im = SQRT_PI_OVER_2 * ((y >= 0) ? (exp(y2) - erfcx(y)) : (erfcx(-y) - exp(y2)));
            return complex(x, im);
        }

        double mz2re = (y - x) * (x + y); // Re(-z^2), being careful of overflow
        double mz2im = -2 * x * y;  // Im(-z^2)
        Complex mz2 = complex(mz2re, mz2im);
    ...
    private static Complex dawsonTaylor(Complex z, Complex mz2) {
        return z.times(sum(1,
                mz2.times(sum(0.6666666666666666666666666666666666666667,
 ...
 */
    ...
    private static Complex dawsonTaylorRealAxis(double x, double y) {
        double x2 = x * x;
        if (x2 > 1600) { // |x| > 40
            double y2 = y * y;
            if (x2 > 25e14) {// |x| > 5e7
                double xy2 = (x * y) * (x * y);
                return complex((0.5 + y2 * (0.5 + 0.25 * y2
            ...
            return prod(1. / (-15 + x2 * (90 + x2 * (-60 + 8 * x2))),
                    complex(x * (33 + x2 * (-28 + 4 * x2)
                                    + y2 * (18 - 4 * x2 + 4 * y2)),
                            y * (-15 + x2 * (24 - 4 * x2)
                                    + y2 * (4 * x2 - 10 - 4 * y2))));
        } else {
            double D = SQRT_PI_OVER_2 * wIm(x);
            double y2 = y * y;
            return complex
                    (D + y2 * (D + x - 2 * D * x2)
    ...
    private static double sinc(double x, double sinx) {
        return (abs(x) < 1e-4) ? (1 - (0.1666666666666666666667) * x * x) : (sinx / x);
    }

    // sinh(x) via Taylor series, accurate to machine precision for |x| < 1e-2
    private static double sinhTaylor(double x) {
        double x2 = x * x;
    ...
    public static Complex w(Complex z) {
        return w(z, DBL_EPSILON);
    }
            ...
            /* Poppe & Wijers suggest using a number of terms
               ...
               I also separate the regions where nu == 2 and nu == 1. */
                ...
                if (x + ya > 4000) { // nu <= 2
                    ...
                    { // w(z) = i/sqrt(pi) / w:
                        double denom = ONE_OVER_SQRT_PI / (wr * wr + wi * wi);
                ...
                if (y < 0) {
                    ...
                    return complex((ya - xs) * (xs + ya), 2 * xs * y).exp().times(2).minus(ret);
                }
                return ret;
            }
  ...
  /* Note: The test that seems to be suggested in the paper is x <
        ...
        if (x < 10) {
            ...
            if (relerr == DBL_EPSILON) { // use precomputed exp(-a2*(n*n)) table
                if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
                    ...
                    final double ax2 = 1.036642960860171859744 * x; // 2*a*x
                    final double exp2ax = 1 + ax2 * (1 + ax2 * (0.5 + 0.166666666666666666667 * ax2));
                    final double expm2ax =
                    ...
                    expx2 = exp(-x * x);
                    final double exp2ax = exp((2 * a) * x);
                    final double expm2ax = 1 / exp2ax;
                    for (int n = 1; ; ++n) {
                        final double coef = EXPA2N2[n - 1] * expx2 / (a2 * (n * n) + y * y);
                        prod2ax *= exp2ax;
                        prodm2ax *= expm2ax;
                ...
                if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
                    ...
                    expx2 = 1 - x2 * (1 - 0.5 * x2); // exp(-x*x) via Taylor
                    for (int n = 1; ; ++n) {
                        final double coef = exp(-a2 * (n * n)) * expx2 / (a2 * (n * n) + y * y);
                        prod2ax *= exp2ax;
                        prodm2ax *= expm2ax;
                        sum1 += coef;
                        sum2 += coef * prodm2ax;
            ...
            final double expx2erfcxy = (y > -6) ? (expx2 * erfcx(y)) : (2 * exp(y * y - x * x));
            if (y > 5) { // imaginary terms cancel
                final double sinxy = sin(x * y);
                ...
                final double coef2 = c * xs * expx2;
                ret = complex(coef1 * cos2xy + coef2 * sinxy * sinc(xs * y, sinxy),
    ...
    private static Complex sum(double x, Complex z) {
        return z.plus(x);
    }

    private static Complex prod(double x, Complex z) {
        return z.times(x);
    }
    ...
    static double erfcxY100(double y100) {
        switch ((int) y100) {
            case 0: {
                double t = 2 * y100 - 1;
    ...
    public static double erfcx(double value) {
        if (value >= 0) {
            ...
            return erfcxY100(400 / (4 + value));
        } else if (value < -26.7) {
            return Double.POSITIVE_INFINITY;
        } else if (value < -6.1) {
            return 2 * exp(value * value);
        } else {
            return 2 * exp(value * value) - erfcxY100(400 / (4 - value));
        }
    }
    ...
    private static double wIm(double value) {
        if (value >= 0) {
      ...
      /* 5-term expansion (rely on compiler for CSE), simplified from:
            ...
            return wImY100(100 / (1 + value), value);
      ...
      /* 5-term expansion (rely on compiler for CSE), simplified from:
                1 / sqrt(pi) / (x-0.5/(x-1/(x-1.5/(x-2/x))))  */
                return ONE_OVER_SQRT_PI * ((value * value) * (value * value - 4.5) + 2) / (value * ((value * value) * (value * value - 5) + 3.75));
            }
            return -wImY100(100 / (1 - value), -value);
        }
    }
